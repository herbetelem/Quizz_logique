...
class SQL_request:
    ...
    def read_question(self, id_question):
        ...
        # * je créer la requete
        self.cursor.execute('SELECT * FROM lh_question WHERE id = ?', id_question)
    ...
    def read_answer(self, id_question):
        ...
        id_question = (id_question,)
        # * je créer la requete
        self.cursor.execute('SELECT * FROM lh_reponse WHERE id_question = ?', id_question)
    ...
    def read_score(self):
        ...
        self.cursor = self.connection.cursor()
        # * je créer la requete
        self.cursor.execute('SELECT * FROM lh_score ORDER BY score DESC')
        # * je formate en tuple mes reponses
    ...
    def create_score(self, name, score):
        ...
        cursor.execute('INSERT INTO lh_score VALUES(?,?,?)', new_user)
    ...
    def verification_user(self, name_user):
        ...
        self.cursor.execute('SELECT * FROM lh_score WHERE name = ?', name_user)
    ...
    def update_user(self, name, score):
        ...
        cursor.execute('UPDATE lh_score SET score = ? WHERE name = ?', update_user)
        connection.commit()
        connection.close()
